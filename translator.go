...
func (t *DefaultRSSTranslator) Translate(feed interface{}) (*Feed, error) {
	...
	dcv := dcFallbackValues(dc)

	result.Title = firstNonEmpty(rss.Title, dcv.Title)

	result.Description = firstNonEmpty(rss.Description, itunesFeedSummary(rss.ITunesExt))

	result.Language = firstNonEmpty(rss.Language, dcv.Language)

	result.Copyright = firstNonEmpty(rss.Copyright, dcv.Rights)

	result.Updated = firstNonEmpty(rss.LastBuildDate, dcv.Date)
	result.UpdatedParsed = rss.LastBuildDateParsed
	if result.UpdatedParsed == nil {
		result.UpdatedParsed = parseDateOrNil(dcv.Date)
	}
	...
	result.Authors = singleAuthorList(result.Author)
...
func (t *DefaultRSSTranslator) translateFeedItem(rssItem *rss.Item) *Item {
	...
	item.Title = firstNonEmpty(rssItem.Title, dcv.Title)

	item.Description = t.translateItemDescription(rssItem)

	item.Content = firstNonEmpty(rssItem.Content, t.atomExtValue(rssItem.Extensions, "content"))
	...
	item.Authors = singleAuthorList(item.Author)
...
func (t *DefaultRSSTranslator) translateFeedLink(rss *rss.Feed) (link string) {
	...
	for _, ex := range t.extensionsForKeys(atomExtensionKeys, rss.Extensions) {
...
func (t *DefaultRSSTranslator) translateFeedFeedLink(rss *rss.Feed) (link string) {
	atomExtensions := t.extensionsForKeys(atomExtensionKeys, rss.Extensions)
...
func (t *DefaultRSSTranslator) translateFeedAuthor(rss *rss.Feed) *Person {
	return rssPersonAuthor(firstNonEmpty(rss.ManagingEditor, rss.WebMaster), rss.DublinCoreExt, itunesFeedAuthor(rss.ITunesExt))
}
...
func (t *DefaultRSSTranslator) translateFeedCategories(rss *rss.Feed) (categories []string) {
	var cats []string
	cats = append(cats, rssCategoryValues(rss.Categories)...)

	cats = append(cats, splitKeywords(itunesFeedKeywords(rss.ITunesExt))...)
...
func (t *DefaultRSSTranslator) translateItemUpdated(rssItem *rss.Item) (updated string) {
	return firstNonEmpty(dcFallbackValues(rssItem.DublinCoreExt).Date, t.atomExtValue(rssItem.Extensions, "updated"))
}
...
func (t *DefaultRSSTranslator) translateItemAuthor(rssItem *rss.Item) *Person {
	if author := rssPersonAuthor(rssItem.Author, rssItem.DublinCoreExt, itunesItemAuthor(rssItem.ITunesExt)); author != nil {
		return author
	}
...
func (t *DefaultRSSTranslator) translateItemCategories(rssItem *rss.Item) (categories []string) {
	var cats []string
	cats = append(cats, rssCategoryValues(rssItem.Categories)...)

	cats = append(cats, splitKeywords(itunesItemKeywords(rssItem.ITunesExt))...)

	if rssItem.DublinCoreExt != nil && rssItem.DublinCoreExt.Subject != nil {
		cats = append(cats, rssItem.DublinCoreExt.Subject...)
	}

	for _, m := range t.extensionsForKeys(atomExtensionKeys, rssItem.Extensions) {
...
func (t *DefaultRSSTranslator) translateItemEnclosures(rssItem *rss.Item) (enclosures []*Enclosure) {
	for _, enc := range rssItem.Enclosures {
		enclosures = append(enclosures, newEnclosure(enc.URL, enc.Length, enc.Type))
	}
	return
}
...
func (t *DefaultAtomTranslator) Translate(feed interface{}) (*Feed, error) {
	...
	result.Links = webLinkHrefs(atomFeed.Links)

	result.Author, result.Authors = atomAuthors(atomFeed.Authors)
...
func (t *DefaultAtomTranslator) translateFeedItem(entry *atom.Entry) *Item {
	...
	item.Links = webLinkHrefs(entry.Links)

	// Published falls back to the update time when absent.
	item.Published = firstNonEmpty(entry.Published, entry.Updated)
	item.PublishedParsed = entry.PublishedParsed
	if item.PublishedParsed == nil {
		item.PublishedParsed = entry.UpdatedParsed
	}

	item.Author, item.Authors = atomAuthors(entry.Authors)

	item.Categories = atomCategories(entry.Categories)

	for _, l := range entry.Links {
		if l.Rel == "enclosure" {
			item.Enclosures = append(item.Enclosures, newEnclosure(l.Href, l.Length, l.Type))
		}
	}
...
func (t *DefaultJSONTranslator) Translate(feed interface{}) (*Feed, error) {
	...
	result.Links = appendNonEmpty(result.Links, jsonFeed.HomePageURL, jsonFeed.FeedURL)
	...
	result.Author, result.Authors = translateJSONAuthors(jsonFeed.Author, jsonFeed.Authors)
...
func (t *DefaultJSONTranslator) translateFeedItem(jsonItem *json.Item) *Item {
	...
	item.Links = appendNonEmpty(item.Links, jsonItem.URL, jsonItem.ExternalURL)
	...
	item.Author, item.Authors = translateJSONAuthors(jsonItem.Author, jsonItem.Authors)
	...
	if jsonItem.Attachments != nil {
		for _, attachment := range *jsonItem.Attachments {
			e := newEnclosure(attachment.URL, "", attachment.MimeType)
